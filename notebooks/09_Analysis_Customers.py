...
display(spark.sql(f"""SELECT
  ...
  COUNT(DISTINCT CASE WHEN is_new_customer THEN a.customer_key END) AS new_customers_last_month,
  COUNT(DISTINCT CASE WHEN NOT is_new_customer AND revenue_usd > 0 THEN a.customer_key END) AS returning_customers,
  ROUND(SUM(revenue_usd) /
        NULLIF(COUNT(DISTINCT CASE WHEN revenue_usd > 0 THEN a.customer_key END), 0), 2) AS avg_revenue_per_customer
